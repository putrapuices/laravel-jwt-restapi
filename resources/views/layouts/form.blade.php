...
<div class="mb-3 row">
    ...
    <div class="col-md-6">
        <div class="row g-2">
            <div class="col-3">
                <input type="number" name="tgl" id="tgl" class="form-control col-md-3 d-inline
 @error('tanggal_lahir') is-invalid @enderror" placeholder="dd" value="{{ old('tgl') ?? $user->tgl ?? '' }}">
            </div>
            <div class="col-5">
                @php
 ...
 @error('tanggal_lahir') is-invalid @enderror" style="vertical-align: baseline;" name="bln" id="bln">
                    @for ($i = 0; $i < 12; $i++) @if ($i+1==(old('bln') ?? $user->bln ?? ''))
 ...
 @error('tanggal_lahir') is-invalid @enderror" name="thn" placeholder="yyyy" value="{{ old('thn') ?? $user->thn ?? ''}}">
            </div>
            @error('tanggal_lahir')
            <span class="invalid-feedback" role="alert">
                <strong>{{ $message }}</strong>
...
<div class="mb-3 row">
    <label for="pekerjaan" class="col-md-3 col-form-label text-md-end">
        Pekerjaan </label>
    <div class="col-md-6">
        <input id="pekerjaan" type="text" class="form-control @error('pekerjaan') is-invalid @enderror" name="pekerjaan" value="{{ old('pekerjaan') ?? $user->pekerjaan ?? '' }}">
        @error('pekerjaan')
        <span class="invalid-feedback" role="alert">
            <strong>{{ $message }}</strong>
        </span>
...
<div class="mb-3 row">
    ...
    <div class="col-md-6">
        <input id="kota" type="text" class="form-control @error('kota') is-invalid @enderror" name="kota" value="{{ old('kota') ?? $user->kota ?? ''}}">
        @error('kota')
        <span class="invalid-feedback" role="alert">
            <strong>{{ $message }}</strong>
        </span>
...
<div class="mb-3 row">
    <label for="gambar_profil" class="col-md-3 col-form-label text-md-end">
        Gambar Profil</label>
    <div class="col-md-6">
        <img id="display_gambar_profil" class="img-thumbnail w-25 mb-2" @if ($tombol=='Daftar' ) src="{{ asset('img/default_profile.jpg') }}" @elseif ($tombol=='Update' ) src="{{ asset('storage/uploads/'.$user->gambar_profil) }}" @endif>
